...
function UserContentProvider({ children }) {
  ...
  const { COLLECTION_URL_BASE } = useBaseUrl();
  const { FILE_SERVER_BASE } = useBaseUrl();

  const { token } = useAuthContext();

  async function fetchProjects() {
    const data = await (await fetch(COLLECTION_URL_BASE + '/projects')).json();
    if (data.success) {
  ...
  async function getDbCollections() {
    ...
    try {
      const response = await (await fetch(COLLECTION_URL_BASE, {
        headers: {
  ...
  async function addProject(
    ...
    try {
      const newProject = await (
        await fetch(COLLECTION_URL_BASE + '/projects', {
          method: 'POST',
          headers: {
  ...
  async function updateProject(
    ...
    try {
      const response = await (
        await fetch(`${COLLECTION_URL_BASE}/projects/${id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + token,
